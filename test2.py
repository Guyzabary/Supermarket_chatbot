...
def ask_openai(text: str) -> str:
    ...
    try:
        ...
        msgs += conversation_state["history"][-50:]
...
class ChatWindow(QMainWindow):
    def __init__(self):
        ...
        self.scroll_area.setLayoutDirection(Qt.LeftToRight)
    ...
    def on_send(self):
        ...
        if st == "wait_for_selection":
            # נסו לקרוא את ה־idx, אחרת נחזיר שגיאה למשתמש
            try:
                idx = int(ui)
            except ValueError:
            ...
            # אם אין כלל pending choices, נוותר בשקט
            ...
            # טווח תקין: 0 עד len(opts)
            if idx < 0 or idx > len(opts):
                self.insert_bot_message("⚠️ אנא בחר מספר תקני מתוך הרשימה.")
                return

            # עכשיו מסירים מה־pending
            ...
            # אם יש עוד מוצרים שממתינים לבחירה
            ...
            # אם סגירת כל הבחירות – ממשיכים לעיבוד הבא
            self._batch_index += 1
            QTimer.singleShot(50, self._process_next_item)
            return

    def _continue_openai_response(self):
        # אם במקרה אין pending input, יוצאים
        if not self._pending_user_input:
            return

        # שולחים ל־OpenAI את ההודעה של המשתמש
        response = ask_openai(self._pending_user_input)

        # אם יש תווית "חושב", מעדכנים אותה, אחרת פשוט שולחים בוט-מסר חדש
