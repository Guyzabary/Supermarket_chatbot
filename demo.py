...
class OpenAIWorker(QThread):
    # נגדיר סיגנל שישלח לממשק את הטקסט של התשובה
    ...
    def run(self):
        # הקוד הזה ירוץ ב־thread נפרד
        try:
            ans = ask_openai(self.prompt)
        except Exception as e:
            ans = f"שגיאה ב־OpenAI: {e}"
        # וכאשר יש תשובה, נפצח אותה חזרה ל־UI-thread
...
class ChatWindow(QMainWindow):
    ...
    def on_send(self):
        ...
        if st == "chat":
            if any(trigger in ui_low for trigger in TRIGGERS):
                ...
                # 1) הצג "חושב..." והחזר לך את התווית
                thinking_label = self.insert_bot_message("🤖 רגע... חושב...", return_label=True)

                # 2) הכנס לקריאה ב־background-thread ושמור אותו על ה־self
                self._openai_worker = OpenAIWorker(ui)
                # 3) קבע שהסיגנל 'finished' יעדכן את התווית
                self._openai_worker.finished.connect(
                    lambda ans: thinking_label.setText(ans)
                )
                # 4) התחל את ה־thread
    ...
    def _continue_openai_response(self):
        ...
        # שמרו את הלייבל של "חושב" כדי לעדכן אותו אחר־כך
        thinking_label = self._thinking_label
        self._thinking_label = None  # כדי שלא נדאג לו עוד

        # אתחול ה־worker ושמירתו על self
        self._openai_worker = OpenAIWorker(self._pending_user_input)
        # כשיסיים, עדכן את ה-label
        self._openai_worker.finished.connect(lambda ans: thinking_label.setText(ans))
        # התחל את הריצה ברקע
        self._openai_worker.start()

    def _complete_openai_response(self):
        answer = ask_openai()
        if self._thinking_label:
            self._thinking_label.setText(answer)
            self._thinking_label = None
