    ...
    def _finish_cart(self):
        ...
        for chain, total in totals.items():
            ...
            for prod in conversation_state["cart_items"]:
                ...
                price_text = f"₪{pr.iloc[0]:.2f}" if not pr.empty else "N/A"
    ...
    def on_send(self):
        ...
        if st == "chat":
            if any(trigger in ui_low for trigger in TRIGGERS):
                ...
                # 2. אחרי חצי שנייה עוברים לבקש כתובת
        ...
        if st == "ask_location":
            ...
            conversation_state["stage"] = "ask_cart"
            self.insert_bot_message("📋 אנא הזן את רשימת המוצרים (מופרדים בפסיקים):")
            return
        ...
        if st == "wait_for_selection":
            ...
            key = conversation_state["pending_order"][conversation_state["pending_index"]]
            opts = conversation_state["pending_choices"][key]

            if idx == 0:
                pass
            elif 1 <= idx <= len(opts):
                conversation_state["cart_items"].append(opts[idx - 1])
            else:
                self.insert_bot_message("⚠️ אנא בחר מספר תקני מתוך הרשימה.")
                return

            del conversation_state["pending_choices"][key]
            conversation_state["pending_index"] += 1

            if conversation_state["pending_index"] < len(conversation_state["pending_order"]):
                nk = conversation_state["pending_order"][conversation_state["pending_index"]]
                nopts = find_product_match(nk)
                options_text = "0. לא לבחור כלום מהרשימה\n" + "\n".join(f"{i + 1}. {o}" for i, o in enumerate(nopts))
                self.insert_bot_message(f"בחר התאמה ל'{nk}':\n{options_text}")
                return
